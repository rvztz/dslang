...
class DSLangParser(Parser):
    ...
    @_('varidx gen_fidx ASGN expr gen_idx_assign')
    def assign(self, p):
        return p
    ...
    @_('READ LPAREN ID RPAREN gen_read', 
        'READ LPAREN varidx gen_fidx gen_idxread RPAREN')
    def read(self, p):
        return p
    ...
    @_('ID LBRCKT validate_arr push_oper_ctx expr RBRCKT pop_oper_ctx gen_vdim arrindex')
    def varidx(self, p):
        return p[0]

    @_('LBRCKT push_oper_ctx expr RBRCKT pop_oper_ctx gen_vdim arrindex')
    def arrindex(self, p):
        pass
    ...
    def add_vid(self, p):
        try:
            var, _ = self.ctx.get_var(p[-1])
            self.ctx.egen.add_operand(var.addr, var.vtype, p[-1])
    ...
    def gen_fidx(self, p):
        try:
            return self.ctx.gen_idxq(False)
        except Exception as e:
            raise SystemExit('DSLang error => ', e)
    ...
    def add_funcv(self, p):
        try:
            ...
            self.ctx.egen.add_operand(fvar.addr, fvar.vtype, fid)
    ...
    def add_params(self, p):
        try:
            pid = self.ctx.funcdir.get(self.ctx.curr_fid).porder[self.ctx.curr_pidx]
            param = self.ctx.funcdir.get(self.ctx.curr_fid).params.get(pid)
            if not param:
